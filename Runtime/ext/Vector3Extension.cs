...
namespace unvs.ext
{
    public static class PhysilcaExt
    {
        ...
        public static void CalculateSlopeDirection(this Vector2 pos, ref CalculateSlopeDirectionResull Result, float direction, float groundCheckDistance = 20f, string layerName = Constants.Layers.WORLD_GROUND)
        {
    ...
    public static class Vector3Extension
    {
        ...
        public static Vector2 ToWorld(this Vector2 position)
        {
            ...
            {
                // Chế độ Orthographic: Z không ảnh hưởng đến phép tính XY
                return Camera.main.ScreenToWorldPoint(position);
            }
            else
            {
                // Chế độ Perspective: Phải tính toán dựa trên tia (Ray)
                Ray ray = Camera.main.ScreenPointToRay(position);

                // Tạo một mặt phẳng ảo nằm ngang tại Z = 0 
                // (Vector3.forward là hướng của mặt phẳng, Vector3.zero là gốc tọa độ)
                Plane xyPlane = new Plane(Vector3.forward, Vector3.zero);

                if (xyPlane.Raycast(ray, out float distance))
                {
                    // Lấy điểm tại khoảng cách mà tia chạm vào mặt phẳng Z=0
                    Vector3 worldPoint = ray.GetPoint(distance);
                    return new Vector2(worldPoint.x, worldPoint.y);
                }

                // Trường hợp tia không chạm mặt phẳng (ví dụ nhìn ra vô cực)
                return Vector2.zero;
            }
        }
        /// <summary>
        ...
        public static GameObject GetClosestTarget(this Vector2 currentPosition, Collider2D[] targets)
        {
            ...
            foreach (Collider2D potentialTarget in targets)
            {
                Vector3 directionToTarget =(Vector2) potentialTarget.transform.position - (Vector2)currentPosition;
                float dSqrToTarget = directionToTarget.sqrMagnitude;
                if (dSqrToTarget < closestDistanceSqr)
                {
                    closestDistanceSqr = dSqrToTarget;
    ...
    public static class Vector2dExtesion
    {
        ...
        public static Vector2[] CreateThicknessDownPolygonFromLines(this Vector2[] lines, float thicknessValue)
        {
            if (lines == null || lines.Length < 2)
            {
                Debug.LogError("Points phải có ít nhất 2 điểm");
                return lines;
            }

            int n = lines.Length;
            Vector2[] newPoints = new Vector2[n * 2];

            // 1. Copy các điểm trên (top edge)
            for (int i = 0; i < n; i++)
            {
                newPoints[i] = lines[i];
            }

            // 2. Tạo các điểm dưới (bottom edge) - offset xuống dưới
            for (int i = 0; i < n; i++)
            {
                newPoints[n + i] = new Vector2(lines[i].x, lines[i].y - thicknessValue);
            }

            // 3. Đảo ngược thứ tự điểm dưới để tạo đa giác kín đúng chiều
            // (để tránh self-intersect và winding order đúng)
            System.Array.Reverse(newPoints, n, n);   // đảo ngược phần bottom

            return newPoints;
        }
        public static Vector2[] CreateThicknessPolygonWithFlatBottomFromLines(this Vector2[] lines, float thicknessValue)
        {
            if (lines == null || lines.Length < 2)
            {
                Debug.LogError("Points phải có ít nhất 2 điểm");
            ...
            int n = lines.Length;
            Vector2[] polygon = new Vector2[n * 2];

            // 1. Tìm điểm thấp nhất (Min Y) của đường line để tính toán cao độ đáy
            float minY = lines[0].y;
            for (int i = 1; i < n; i++)
            {
                if (lines[i].y < minY) minY = lines[i].y;
            }

            // Cao độ của đáy phẳng (nằm dưới điểm thấp nhất một khoảng thickness)
            float bottomY = minY - thicknessValue;

            // 2. Điền các điểm cho cạnh trên (từ trái sang phải)
            for (int i = 0; i < n; i++)
            {
                polygon[i] = lines[i];
            }

            // 3. Điền các điểm cho cạnh dưới (từ phải sang trái)
            // Việc đi ngược lại giúp tạo thành một vòng khép kín không tự cắt
            for (int i = 0; i < n; i++)
            {
                int lineIndex = n - 1 - i; // Đi ngược từ n-1 về 0
                int polyIndex = n + i;     // Đi tiếp từ n đến 2n-1

                // Đáy phẳng: X lấy từ line, Y cố định tại bottomY
                polygon[polyIndex] = new Vector2(lines[lineIndex].x, bottomY);
            }

            return polygon;
        }
        public static Vector2[] CreateThicknessPolygonWithOneLineFlatBottomFromLines(this Vector2[] lines, float thicknessValue)
        {
            if (lines == null || lines.Length < 2)
            {
                Debug.LogError("Points phải có ít nhất 2 điểm");
            ...
            int n = lines.Length;
            // Tổng số điểm: n điểm trên + 2 điểm đáy = n + 2
            Vector2[] polygon = new Vector2[n + 2];

            // 1. Tìm cao độ thấp nhất để tính đáy
            float minY = lines[0].y;
            for (int i = 1; i < n; i++)
            {
                if (lines[i].y < minY) minY = lines[i].y;
            ...
            // 2. Copy toàn bộ cạnh trên (n điểm)
            for (int i = 0; i < n; i++)
            {
                polygon[i] = lines[i];
            }

            // 3. Thêm 2 điểm để chốt cái đáy phẳng
            // Điểm chốt dưới bên phải (từ điểm cuối của cạnh trên hạ thẳng xuống)
            polygon[n] = new Vector2(lines[n - 1].x, bottomY);

            // Điểm chốt dưới bên trái (về lại tọa độ X đầu tiên để đóng vòng)
            polygon[n + 1] = new Vector2(lines[0].x, bottomY);

            return polygon;
        }
        public static Vector2[] CreateThicknessPolygonUsingNormals(this Vector2[] lines, float thickness)
        {
            ...
            // --- BƯỚC 1: TÍNH VECTOR PHÁP TUYẾN TRUNG BÌNH TẠI MỖI ĐỈNH ---
            for (int i = 0; i < n; i++)
            {
                Vector2 normal;
                if (i == 0) // Điểm đầu
                {
                    normal = GetNormal(lines[0], lines[1]);
                }
                else if (i == n - 1) // Điểm cuối
                {
                    normal = GetNormal(lines[n - 2], lines[n - 1]);
                }
                else // Các điểm ở giữa (Tính trung bình pháp tuyến của 2 đoạn kề)
                {
                    Vector2 n1 = GetNormal(lines[i - 1], lines[i]);
                    Vector2 n2 = GetNormal(lines[i], lines[i + 1]);
                    normal = (n1 + n2).normalized;

                    // Kỹ thuật Miter: Điều chỉnh độ dài pháp tuyến tại góc để độ dày đồng nhất
                    float dot = Vector2.Dot(n1, normal);
                    if (dot > 0.001f) normal /= dot;
                }
                normals[i] = normal;
            }

            // --- BƯỚC 2: TẠO CÁC ĐIỂM CHO ĐA GIÁC ---
            // Cạnh trên (Đường line gốc)
            for (int i = 0; i < n; i++)
            {
                polygon[i] = lines[i];
            }

            // Cạnh dưới (Offset theo pháp tuyến)
            for (int i = 0; i < n; i++)
            {
                // Đi ngược từ n-1 về 0 để đóng vòng kín
                int lineIdx = n - 1 - i;
                int polyIdx = n + i;

                // Offset dựa trên pháp tuyến và thickness (âm hay dương tùy bạn truyền vào)
                polygon[polyIdx] = lines[lineIdx] + (normals[lineIdx] * thickness);
            }

            return polygon;
        }

        // Hàm bổ trợ tính pháp tuyến của một đoạn thẳng (xoay 90 độ)
