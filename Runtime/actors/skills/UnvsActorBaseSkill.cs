...
namespace unvs.actor.skills
{
    ...
    public class ActorDefaultSkill : AbstractActionBaseSkill
    {
        ...
        public override void OnPerform(Action OnCompleted = null)
        {
            ...
            coll.CalculateSlopDirection(ref r, Direction.x,20f);
            var forceDown = 0f;
            if (r.IsHit)
            {
                if (r.hitCollider.tag == Constants.Tags.RIGID_BOX)
                {
                    var rb = r.hitCollider.GetComponent<UnvsRigidObject>();
                    if (rb != null)
                    {
                        ...
                        if (vY < 0)
                        {
                            // Ép hướng di chuyển của nhân vật đi xuống nhanh hơn thang máy một chút
                            // Điều này giúp GroundCheck luôn IsHit = true mà không cần dùng SetParent
                            forceDown = vY - 2f;
                        }
                    }
            ...
            this.MoveStep(Owner.transform, r.slopeDir, forceDown);
        }

        public virtual bool IsOnRigBox()
        {
        ...
        public void MoveStep(Transform transform, Vector2 slopeDirection, float forceDown)
        {

            if (Application.isPlaying)
            {
                var step = Direction * base.CurrentSpeed + new Vector2(0, forceDown);
                //if (CurrentSpeed > 0)
                {
                    ...
                    //else
                        ...
                        transform.position += (Vector3)step * Time.deltaTime;
                }
                //transform.position += (Vector3)step * Time.deltaTime;
            }
